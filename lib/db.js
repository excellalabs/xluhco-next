...
async function createLink(linkUrl, slug = "") {
    ...
    const ogsOptions = {url: linkUrl, onlyGetOpenGraphInfo: true};

    const opengraph_results = await ogs(ogsOptions, (error, results, response) => {return results})
    ...
    const result =  client.createEntity(newEntity)
            .then((s) => client.getEntity(process.env.AZURE_TABLES_PARTITION_KEY, s))
            .then((s) => {s})
            .catch((e) => e)
...
async function getLink(slug) {
    const linkRecord = await client.getEntity(process.env.AZURE_TABLES_PARTITION_KEY, slug)
                                    .catch((e)=> e );
    return {
      "linkUrl": linkRecord.linkUrl,
      "slug": linkRecord.rowKey
    };
}
